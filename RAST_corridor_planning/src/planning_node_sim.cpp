...
queue<Eigen::Vector3d>    position_queue_;
queue<Eigen::Quaternionf> quaternion_queue_;
...
/**
 * @brief Get the Point Spatial Index In DSP Map object
 *
 * @param p
 * @param map_center
 * @return int
...
void trajectoryCallback(const ros::TimerEvent& e) {
  ...
  Eigen::Vector3d planning_start_v = Eigen::Vector3d::Zero();  // current_vel_;
  Eigen::Vector3d planning_start_a = Eigen::Vector3d::Zero();  // current_vel_;
  ...
  if (in_safety_mode) {
    ...
    } else if (!trajectory_piece.empty() && (p_store_for_em - current_pos_).norm() > 1.0) {
      // Tracking error too large
      PVAYPoint temp_p;
      temp_p.position     = 0.5 * p_store_for_em + 0.5 * current_pos_;
  ...
  //    }
  //
  //    vis_->visualizeAstarPath(searched_points_to_show, 89, 1.0, 0.1, 0.1, 1.0, 0.05);
  ...
  if (result.size() > 1 && result.size() < 10) {  // at least two nodes to build a corridor
    ...
    vis_->visualizeAstarPath(points, 0, 0.8, 0.3, 0.4, 1.0, 0.2);
    ...
    vis_->visualizeAstarPath(a_star_traj_points_to_show, 1, 0.1, 0.9, 0.2, 1.0, 0.1,
    ...
    vector<Eigen::Vector3d> points;
    vis_->visualizeAstarPath(points, 0, 0.8, 0.3, 0.4, 1.0, 0.2, visualization_msgs::Marker::POINTS,
                            true);
    vis_->visualizeAstarPath(points, 1, 0.1, 0.9, 0.2, 1.0, 0.1,
...
// TODO(@siyuan): move above decomp_ros_msgs functions to a seperate file
 ...
 *
 * @param msg
 * @param planning_start_map_center
 * @return true
 * @return false
 */
bool optimizationInCorridors(const decomp_ros_msgs::DynPolyhedronArray msg,
...
void setpointCallback(const ros::TimerEvent& e) {
  ...
  tr.translation.x = current_pos_.x();
  tr.translation.y = current_pos_.y();
  tr.translation.z = current_pos_.z();
  ...
  vel.linear.x = current_vel_.x();
  vel.linear.y = current_vel_.y();
  vel.linear.z = current_vel_.z();
  ...
  pose_sp_msg.pose.position.x = p_store_for_em(0) - current_pos_.x();
  pose_sp_msg.pose.position.y = p_store_for_em(1) - current_pos_.y();
  pose_sp_msg.pose.position.z = p_store_for_em(2) - current_pos_.z();
  ...
  if (fabs(current_pos_.x() - goal_x) < 2.f && fabs(current_pos_.y() - goal_y) < 2.f) {
...
void simPoseCallback(const geometry_msgs::PoseStamped& msg) {
  if (!state_locked) {
    ...
    current_pos_.z() = msg.pose.position.z;

    current_qua_.x() = msg.pose.orientation.x;
    current_qua_.y() = msg.pose.orientation.y;
    current_qua_.z() = msg.pose.orientation.z;
    current_qua_.w() = msg.pose.orientation.w;

    position_queue_.push(current_pos_);
    quaternion_queue_.push(current_qua_);
  ...
  axis.y()                       = 0.0;
  axis.z()                       = sin(-M_PI / 4.0);
  Eigen::Quaternionf rotated_att = current_qua_ * axis;
}

void simVelocityCallback(const geometry_msgs::TwistStamped& msg) {
  current_vel_.x() = msg.twist.linear.x;
  current_vel_.y() = msg.twist.linear.y;
  current_vel_.z() = msg.twist.linear.z;
  ...
  if (init_v_flag) {
    ...
    current_acc_(0) = (current_vel_(0) - last_vx) / delt_t;
    current_acc_(1) = (current_vel_(1) - last_vy) / delt_t;
    current_acc_(2) = (current_vel_(2) - last_vz) / delt_t;
  ...
  last_vx   = current_vel_(0);
  last_vy   = current_vel_(1);
  last_vz   = current_vel_(2);
}
...
int main(int argc, char** argv) {
  ...
  vis_.reset(new Visualizer(n, frame_id));
