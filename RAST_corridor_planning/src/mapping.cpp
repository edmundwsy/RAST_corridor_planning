...
ros::Publisher future_risk_pub, future_risk_full_array_pub, current_marker_pub, fov_pub,
    cluster_status_pub, risk_2dmap_pub;
...
void showFOV(Eigen::Vector3d    &position,
...
void                                cloudCallback(const sensor_msgs::PointCloud2ConstPtr &cloud) {
  ...
  //         ROS_INFO_THROTTLE(3.0, "cloud mismatch time = %lf", cloud->header.stamp.toSec() -
  //         time_stamp_pose);
  ...
  // "<<uav_position.z()<<std::endl; std::cout << "position queue
  // size="<<uav_position_global_queue.size()<<std::endl;

  if (!my_map.update(useful_point_num, 3, point_clouds, uav_position.x(), uav_position.y(),
                                                    uav_position.z(), this_time, uav_att.w(), uav_att.x(), uav_att.y(),
                                                    uav_att.z())) {
    return;
  }
  ...
  my_map.getOccupancyMapWithRiskMaps(occupied_num, cloud_to_publish, &risk_maps[0][0],
                                                                    0.2);  // 0.25
  ...
  //            int index_this = z_index_to_show*MAP_WIDTH_VOXEL_NUM*MAP_LENGTH_VOXEL_NUM +
  //            j*MAP_WIDTH_VOXEL_NUM + i;
  //
  //            for(int n=0; n<RISK_MAP_NUMBER; ++n){
  //                pcl::PointXYZRGB p_this;
  //                float x_offset = (float)n * 12.f; //Used to show prediction at different times
  //                in one map
  //
  //                my_map.getVoxelPositionFromIndexPublic(index_this, p_this.x, p_this.y,
  //                p_this.z); p_this.x += x_offset;
  //
...
int main(int argc, char **argv) {
  ...
  future_risk_full_array_pub =
      n.advertise<std_msgs::Float32MultiArray>("future_risk_full_array", 1, true);
