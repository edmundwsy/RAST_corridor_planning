...
typedef struct Node {
  ...
  Node(float             time_stamp,
       ...
       float             vz,
       Node *            father,
       RectangleEnvelope envelope_init) {
    this->time_stamp = time_stamp;
    this->x          = x;
    this->y          = y;
    this->z          = z;
...
class Astar {
 public:
  ...
  void setIfSampleZDirection(bool if_sample_z_acc) { sample_z_acc = if_sample_z_acc; }
  ...
  void setHeightLimit(bool if_use_height_limit, float limit_max, float limit_min) {
    ...
    if (if_use_height_limit) {
      cout << " \033[1;33m Kinodynamic A*: To use height limit. updateMapCenterPosition() must be "
              "operated every time. \033[0m"
           << endl;
    }
  }

  void setMaximumVelAccAndStep(float v_max_xy_set,
                               float v_max_z_set,
                               float a_max_xy_set,
                               float a_max_z_set,
                               float a_step_set) {
  ...
  void search(Node *          start_p_v_set,
              Node *          end_pos_set,
              float           start_time_this,
              float           safety_distance_this,
              float           reference_direction_angle_this,
              float *         risk_map_in,
              vector<Node *> &result) {
  ...
  void findCorridors(vector<Corridor *> &corridors, int pattern = 1, float expand_step = 0.2) {
    /// Pattern 0: Expand the corridor to xyz directions at the same until the safety condition is
    /// not satisfied Pattern 1: Use Pattern 0 first. When Pattern 0 fails, close expanding
    /// direction z, and then y, and then x; Pattern 2: Expand by order {x_positive, x_negative,
    /// y_positive, y_negative, z_positive, z_negative} in each
    ...
    for (int node_seq = 1; node_seq < result_path_reversed.size(); ++node_seq) {
      auto *corridor_this =
          new Corridor(result_path_reversed[node_seq - 1], result_path_reversed[node_seq]);
      ...
      vector<bool> forbidden_direction = {false, false, false,
                                          false, false, false};  // for pattern 2
      ...
      while (step_counter < 30) {
        ...
        expandEnvelope(result_path_reversed[node_seq]->envelope, envelope_expanded_this,
                       expanding_distance);

        if (!checkIfEnvelopeSafe(envelope_expanded_this, risk_limitation_corridor,
                                 risk_limitation_single_voxel)) {
  ...
  void findNearestPoint2DOnALine(float  A_x,
                                 float  A_y,
                                 float  B_x,
                                 float  B_y,
                                 float  P_x,
  ...
  float vectorNorm(float x, float y, float z) { return sqrt(x * x + y * y + z * z); }

  float vectorSquareNorm(float x, float y, float z) { return x * x + y * y + z * z; }

  bool ifPointInEnvelope(float x, float y, float z, RectangleEnvelope &envelope) {
    for (const auto &e : envelope.surfaces) {
      float dot = (x - e.point.x) * e.normal.x + (y - e.point.y) * e.normal.y +
                  (z - e.point.z) * e.normal.z;
      //            if(dot > 0.f){ //outside
      //                // check distance
      //                float vector_xe_length = vectorNorm(x - e.point.x, y - e.point.y, z -
      //                e.point.z); float angle = acos(dot/vector_xe_length); //vector normal was
      //                normalized float outside_distance = vector_xe_length * sin(angle);
      //
  ...
  void expandEnvelope(RectangleEnvelope &envelope,
                      RectangleEnvelope &envelope_expanded,
    ...
    float center_x = (envelope.vertexes[0].x + envelope.vertexes[2].x + envelope.vertexes[4].x +
                     ...
                     4.f;
    float center_y = (envelope.vertexes[0].y + envelope.vertexes[2].y + envelope.vertexes[4].y +
                      envelope.vertexes[6].y) /
                     4.f;
    ...
    if (envelope.vertexes[0].x - envelope.vertexes[2].x != 0.f) {
      theta = atan2(envelope.vertexes[0].y - envelope.vertexes[2].y,
                    envelope.vertexes[0].x - envelope.vertexes[2].x);
    }
  ...
  bool findRectangleEnvelope(
      Node *current_node, float ax, float ay, float az, float t, RectangleEnvelope &envelope) {
    ...
    float PA_PB_cos = ((P_x - A_x) * (P_x - B_x) + (P_y - B_y) * (P_y - A_y)) /
                      sqrt(PA_square_length) / sqrt(PB_square_length);
    ...
    if (fabs(PA_PB_cos) < 0.9f) {
      ...
      float length               = sqrt(norm_vertical_AB_1_x * norm_vertical_AB_1_x +
                          norm_vertical_AB_1_y * norm_vertical_AB_1_y);
    ...
    //        if(fabs(A_x - B_x) + fabs(A_y - B_y) < 0.001f || fabs(A_prime_x - B_x) +
    //        fabs(A_prime_y - B_y) < 0.001f
    //            || fabs(B_prime_x - B_x) + fabs(B_prime_y - B_y) < 0.01f || fabs(A_prime_x - A_x)
    //            + fabs(A_prime_y - A_y) < 0.01f){ cout <<"WTF!!!! A_x=" << A_x <<", A_y=" << A_y
    //            <<", B_x="<< B_x <<", B_y=" <<B_y << endl; cout <<"A_prime_x=" << A_prime_x <<",
    //            A_prime_y=" << A_prime_y <<", B_prime_x="<< B_prime_x <<", B_prime_y=" <<B_prime_y
    //            << endl; cout <<"P_x=" << P_x <<", P_y=" << P_y <<", P_prime_x="<< P_prime_x <<",
    //            P_prime_y=" <<P_prime_y << endl; cout << "t=" <<t <<", vx="<< current_node->vx <<
    //            ", ax="<<ax <<", vy="<< current_node->vy << ", ay="<<ay <<endl;
    //        }
    ...
    if (fabs(az) > 0.0001f) {
      ...
      if (t_vz_zero > 0.f && t_vz_zero < t) {
        z_potential_min_max_P_z =
            A_z + current_node->vz * t_vz_zero + 0.5f * az * t_vz_zero * t_vz_zero;
        z_potential_min_max_P_x =
            A_x + current_node->vx * t_vz_zero + 0.5f * ax * t_vz_zero * t_vz_zero;
        z_potential_min_max_P_y =
            A_y + current_node->vy * t_vz_zero + 0.5f * ay * t_vz_zero * t_vz_zero;
      }
    }
    ...
    p1.point.y         = A_y;
    p1.point.z         = 0.f;
    p1.normal.x        = P_prime_x - P_x;
    p1.normal.y        = P_prime_y - P_y;
    p1.normal.z        = 0.f;
    float norm_length1 = vectorNorm(p1.normal.x, p1.normal.y, p1.normal.z);
    p1.normal.x /= norm_length1;
    p1.normal.y /= norm_length1;
    p1.normal.z /= norm_length1;
    ...
    p2.normal.x /= norm_length2;
    p2.normal.y /= norm_length2;
    p2.normal.z /= norm_length2;
    envelope.surfaces.push_back(p2);

    Plane3D p3;
    p3.point.x         = B_prime_x;
    p3.point.y         = B_prime_y;
    p3.point.z         = 0.f;
    p3.normal.x        = P_x - P_prime_x;
    p3.normal.y        = P_y - P_prime_y;
    p3.normal.z        = 0.f;
    float norm_length3 = vectorNorm(p3.normal.x, p3.normal.y, p3.normal.z);
    p3.normal.x /= norm_length3;
    p3.normal.y /= norm_length3;
    p3.normal.z /= norm_length3;
    envelope.surfaces.push_back(p3);

    Plane3D p4;
    p4.point.x         = A_prime_x;
    p4.point.y         = A_prime_y;
    p4.point.z         = 0.f;
    p4.normal.x        = A_x - B_x;
    p4.normal.y        = A_y - B_y;
    p4.normal.z        = 0.f;
    float norm_length4 = vectorNorm(p4.normal.x, p4.normal.y, p4.normal.z);
    p4.normal.x /= norm_length4;
    p4.normal.y /= norm_length4;
    p4.normal.z /= norm_length4;
    envelope.surfaces.push_back(p4);

    Plane3D p5;
    p5.point.x         = z_min_point.x;
    p5.point.y         = z_min_point.y;
    p5.point.z         = z_min_point.z;
    p5.normal.x        = 0.f;
    p5.normal.y        = 0.f;
    p5.normal.z        = -1.f;
    float norm_length5 = vectorNorm(p5.normal.x, p5.normal.y, p5.normal.z);
    p5.normal.x /= norm_length5;
    p5.normal.y /= norm_length5;
    p5.normal.z /= norm_length5;
    envelope.surfaces.push_back(p5);

    Plane3D p6;
    p6.point.x         = z_max_point.x;
    p6.point.y         = z_max_point.y;
    p6.point.z         = z_max_point.z;
    p6.normal.x        = 0.f;
    p6.normal.y        = 0.f;
    p6.normal.z        = 1.f;
    float norm_length6 = vectorNorm(p6.normal.x, p6.normal.y, p6.normal.z);
    p6.normal.x /= norm_length6;
    p6.normal.y /= norm_length6;
    p6.normal.z /= norm_length6;
 ...
 private:
    ...
    for (auto const &ax : a_sample_vector_x) {
      for (auto const &ay : a_sample_vector_y) {
        for (auto const &az : a_sample_vector_z) {
          ...
          if (!getMotionPrimitive(current_node, ax, ay, az, primitive_this_endpoint,
                                  envelope_this)) {
            continue;
          }
          ...
          if (current_node->time_stamp < 0.01f && fabs(reference_direction_angle) < M_PIf32) {
            weight += 0.1f * fabs(reference_direction_angle -
                                  atan2(primitive_this_endpoint.y - current_node->y,
                                        primitive_this_endpoint.x - current_node->x));
          }

          checkPoint(primitive_this_endpoint.x, primitive_this_endpoint.y,
                     primitive_this_endpoint.z, primitive_this_endpoint.vx,
                     primitive_this_endpoint.vy, primitive_this_endpoint.vz, current_node, weight,
                     envelope_this);
        }
      }
    }
  ...
  bool getMotionPrimitive(Node *             current_node,
                          float              ax,
                          float              ay,
                          float              az,
                          TrajPoint &        primitive_end_point,
                          RectangleEnvelope &envelope) {
    ...
    primitive_end_point.x  = current_node->x + current_node->vx * time_step_node +
                            0.5f * ax * time_step_node * time_step_node;
    primitive_end_point.y = current_node->y + current_node->vy * time_step_node +
                            0.5f * ay * time_step_node * time_step_node;
    primitive_end_point.z = current_node->z + current_node->vz * time_step_node +
                            0.5f * az * time_step_node * time_step_node;

    searched_point_vector.push_back(primitive_end_point);

    if (fabs(primitive_end_point.x - current_node->x) +
            fabs(primitive_end_point.y - current_node->y) +
            fabs(primitive_end_point.z - current_node->z) <
        0.01) {
      return false;
    }
    ...
    static vector<float> primitive_envelope_expand_distance = {
        safety_distance,           0, safety_distance, safety_distance, half_safety_distance_for_z,
        half_safety_distance_for_z};
  ...
  void checkPoint(float              x,
                  ...
                  float              g,
                  RectangleEnvelope &envelope) {
    if (!checkNodeValid(x, y, z, vx, vy, vz)) return;

    //        std::chrono::high_resolution_clock::time_point tic =
    //        std::chrono::high_resolution_clock::now();
    if (!checkIfEnvelopeSafe(envelope, risk_limitation_motion_primitive,
                             risk_limitation_single_voxel))
      return;
    //        std::chrono::high_resolution_clock::time_point toc =
    //        std::chrono::high_resolution_clock::now(); std::cout << "Check: "
    //                  << 1.0e-3 * std::chrono::duration_cast<std::chrono::microseconds>(toc -
    //                  tic).count()
    //                  << "ms" << std::endl;
    ...
    if (ifContainedInVector(&close_list, x, y, z, vx, vy, vz) != -1) return;
    ...
    if ((index = ifContainedInVector(&open_list, x, y, z, vx, vy, vz)) != -1) {
      ...
      Node *point =
          new Node(father->time_stamp + time_step_node, x, y, z, vx, vy, vz, father, envelope);
  ...
  void calculateGHF(Node *sNode, Node *eNode, float g) {
    // Estimated time
    float h = sqrt((sNode->x - eNode->x) * (sNode->x - eNode->x) +
                   (sNode->y - eNode->y) * (sNode->y - eNode->y) +
                   (sNode->z - eNode->z) * (sNode->z - eNode->z)) /
              v_max_xy;
  ...
  bool checkNodeValid(Node *node) const {
    if (node->x < -map_length_half || node->x > map_length_half || node->y < -map_width_half ||
        node->y > map_width_half || node->z < -map_height_half || node->z > map_height_half) {
      return false;
    }
  ...
  bool checkNodeValid(float &x, float &y, float &z, float &vx, float &vy, float &vz) const {
    if (x < -map_length_half || x > map_length_half || y < -map_length_half ||
        y > map_length_half || z < -map_length_half || z > map_length_half) {
      return false;
    }
  ...
  bool checkNodeOnBoundary(Node *node, float boundary_width = 1.0) const {
    if (node->x < -map_length_half + boundary_width ||
  ...
  bool checkIfEnvelopeSafe(RectangleEnvelope &envelope,
                           float              risk_threshold,
                           float              risk_threshold_one_voxel) {
    // height_max_limit
    if (use_height_limit) {
  ...
  bool getEnvelopeRectangleIndexes(
      RectangleEnvelope &envelope,
      vector<int> &      indexes) {  // indexes: dimension 1: position, dimension 2: time
    /// Match time
    ...
    static const int z_change_storage_taken =
        MAP_WIDTH_VOXEL_NUM * MAP_LENGTH_VOXEL_NUM * RISK_MAP_NUMBER;  // Order: zyxt
    ...
    for (int i = 0; i <= x_voxels_num; ++i) {
      ...
      for (int j = 0; j <= y_voxels_num; ++j) {
        ...
        for (int k = 0; k < z_voxels_num; ++k) {
          ...
          int index = z_index_this * z_change_storage_taken +
                      y_index_this * y_change_storage_taken +
                      x_index_this * x_change_storage_taken + time_stamp_index_to_check;
  ...
  int ifContainedInVector(vector<Node *> *Nodelist,
                          float &         x,
                          float &         y,
                          float &         z,
                          float &         vx,
    ...
    for (int i = 0; i < Nodelist->size(); i++) {
      float same_node_threshold =
          VOXEL_RESOLUTION *
          std::min((fabs(vx) + fabs(vy) + fabs(vz) + 0.0001f) * time_step_node, 1.f);
  ...
  static bool fourDIndexToOneDIndex(const int &x_index,
                                    const int &y_index,
                                    const int &z_index,
    ...
    /// Get Index
    static const int z_change_storage_taken =
        MAP_WIDTH_VOXEL_NUM * MAP_LENGTH_VOXEL_NUM * RISK_MAP_NUMBER;
    static const int y_change_storage_taken = MAP_LENGTH_VOXEL_NUM * RISK_MAP_NUMBER;
    static const int x_change_storage_taken = RISK_MAP_NUMBER;

    index = z_index * z_change_storage_taken + y_index * y_change_storage_taken +
            x_index * x_change_storage_taken + time_index;
  ...
  static bool compare(Node *n1, Node *n2) { return n1->f < n2->f; }

  static bool nodeEqual(Node *n1, Node *n2, float threshold = 1.f) {
    if (fabs(n1->x - n2->x) < threshold && fabs(n1->y - n2->y) < threshold &&
        fabs(n1->z - n2->z) < threshold) {
      return true;
    } else {
      return false;
    }
